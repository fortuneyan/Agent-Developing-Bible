...
## 10.9 A2A协议下的多Agent监控

2025-2026年，随着A2A（Agent-to-Agent）协议的普及，多Agent系统的监控面临新的挑战——当Agent之间通过标准化协议通信时，传统的"函数调用追踪"不够用了。

### 10.9.1 跨Agent链路追踪

A2A协议下的调用链路不再是"函数A→函数B"，而是"Agent A → Agent B → Agent C"，每个Agent可能运行在不同的进程、甚至不同的机器上。

**关键挑战**：

- **Trace ID传递**：需要在A2A消息中携带Trace ID，才能串联完整链路
- **跨服务延迟**：Agent间网络通信的延迟需要单独监控
- **责任归属**：当任务失败时，需要知道是哪个Agent的哪一步出了问题
...
a2a_message = {
    ...
    "trace_id": "trace-abc-123",        # 全局追踪ID
    "parent_span_id": "span-agent-a",   # 父级Span
    "origin_agent": "planner-agent",    # 发起方
    "target_agent": "data-agent",       # 目标方
    "payload": {...}
}
```
...
## 10.10 2026年Agent观测平台全景

第十章讲了自建监控的方案，但2026年这个领域已经出现了成熟的商业化/开源平台。

### 10.10.1 主流平台对比

| 平台 | 类型 | 核心能力 | 适用场景 |
|------|------|---------|---------|
| **LangSmith v2** | SaaS | 链路追踪、Prompt版本管理、数据集评估 | LangChain/LangGraph项目 |
| **AgentOps** | SaaS | 成本追踪、Session分析、A/B测试 | 多框架通用 |
| **Arize Phoenix** | 开源+SaaS | 嵌入可视化、Trace分析、LLM评估 | 需要深度数据分析 |
| **Helicone** | 开源+SaaS | API代理、缓存、成本分析 | 成本敏感型项目 |
| **Langfuse** | 开源 | Trace、Prompt管理、评分 | 私有化部署需求 |

### 10.10.2 LangSmith v2 的新能力

LangSmith 在 v2 版本做了重大升级：

- **Prompt版本管理**：像Git管理代码一样管理Prompt，支持diff、回滚、分支
- **数据集评估**：上传测试集，自动跑评估，生成报告
- **Trace可视化**：图形化展示Agent的完整执行链路，包括每个节点的输入输出和耗时

**我们团队的实际使用感受**：

> LangSmith 最值钱的不是监控，而是"数据集评估"。我们每次改Prompt之前，先跑一遍测试集看当前表现，改完再跑一遍对比。这样改Prompt就不再是"凭感觉"，而是"看数据"。

### 10.10.3 AgentOps：成本追踪的最佳选择

如果你最关心的是"这个Agent到底花了多少钱"，AgentOps是目前最直观的选择。
...
## 10.11 2026年多Agent治理最佳实践

基于最新的工具和实践，我们总结了几条2026年的治理建议：

### 10.11.1 可观测性优先于功能

> **先上监控，再上功能。**

我们见过太多团队先把Agent功能做了一大堆，然后才想起来加监控。结果出了问题根本不知道从哪查起。

**最小可观测性配置**：

1. 每次LLM调用记录：模型、输入tokens、输出tokens、延迟、成本
2. 每个Agent的执行结果：成功/失败、重试次数、最终输出
3. 每个Session的总成本
...
```
第一层：输入过滤（Prompt注入检测、敏感词过滤）
第二层：执行控制（工具权限、资源限制、超时熔断）
第三层：输出审查（敏感信息过滤、合规检查）
```

这三层缺一不可。我们曾经只做了第一层和第三层，结果一个Agent在执行过程中调用了不该调的数据库——因为第二层的工具权限没配好。
...
*下一章，我们将探讨流程编排——Agent系统的"神经系统"。当单个Agent的能力不够用时，如何通过编排让多个组件协同工作？*
