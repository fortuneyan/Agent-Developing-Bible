...
短期记忆的管理在 2025-2026 年出现了一些专门化的框架，不再是"自己用 Redis 存"这么简单了。

### 6.9.1 Mem0：智能记忆层

Mem0 的核心理念是：**记忆不应该只是"存储"，而应该"理解什么值得记"**。

**核心能力**：

- **自动去重**：用户说了三遍"我喜欢吃辣"，只记一条
- **冲突解决**：用户先说"我住北京"，后说"我搬上海了"，自动更新
- **重要性评分**：不是所有对话都值得记，Mem0 会自动评估
...
# 添加记忆（自动去重和冲突解决）
m.add("我喜欢吃辣", user_id="user_001")
m.add("我住在北京", user_id="user_001")
m.add("我搬到上海了", user_id="user_001")  # 自动覆盖"住在北京"

# 检索相关记忆
memories = m.search("用户住在哪里", user_id="user_001")
# 返回: "用户住在上海"（不是北京）
```

**我们试用后的感受**：Mem0 最大的价值是"帮你省去了自己写去重和冲突解决逻辑的时间"。如果你之前用 Redis 存 Session Memory，换 Mem0 能省掉至少 200 行代码。

### 6.9.2 Zep：长期记忆平台

Zep 的定位更全面——它不只是存对话，而是构建完整的"用户画像"。

**核心能力**：

- **事实提取**：自动从对话中提取用户的事实（喜好、习惯、背景）
- **对话摘要**：自动生成长期对话摘要
- **时间衰减**：旧记忆的权重自动降低

### 6.9.3 Graph-based Memory：基于图谱的短期记忆

当对话中涉及大量实体（人名、项目名、技术名词）时，简单的消息列表不够用了。Graph-based Memory 用知识图谱来维护对话中的实体关系。

**适用场景**：

- 项目管理对话（涉及多个项目、人员、依赖关系）
- 技术支持对话（涉及多个设备、故障、解决方案）
- 医疗问诊（涉及症状、病史、用药）
...
基于上面的各种方案，我们总结了一个实用的记忆架构选型路径：

```
Q1：你的 Agent 需要什么级别的记忆？
  → 只记当前会话 → Redis / 内存字典
  → 需要跨会话记忆 → Mem0 / Zep
  → 需要实体关系追踪 → Graph-based Memory
...
Q3：数据量？
  → < 1000 会话 → Redis 足够
  → 1000-10000 会话 → Redis Cluster
  → > 10000 会话 → 专用记忆框架（Mem0/Zep）
```

**一句话建议**：

> 不要过早优化。我们第一个 Agent 项目用了整整一周搭 Mem0，后来发现用 Redis 字典就够用了。Mem0 的价值在"智能去重和冲突解决"——如果你没有这个问题，就别急着上。

---

*下一章，我们将探讨 Agent 的"长期记忆"——不是记住刚才说了什么，而是记住你是谁、你喜欢什么、你上次做了什么。这才是 Agent 从"工具"变成"伙伴"的关键一步。*
